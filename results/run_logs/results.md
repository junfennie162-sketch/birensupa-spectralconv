...
**正式公开集成绩（2026-08-03 · freeze_r9 · 评测报告 v8）**

| 项 | 值 |
|---|---|
| 数据 | `fno_ns/data/navier_stokes_v1e-3_N1200_T20.pt`（HF 公开 NS64） |
| 划分 | n_train=1000 / n_test=128，seed=`20260722` |
| 训练 | … → sched_samp_r5 → R8 soft → **freeze_r9**（冻 spectral 抛光）；bs=16 |
| test relative L2 | **0.03530218452215195**（`freeze_r9` promote） |
...
| 场景 | relative L2 | 说明 |
|---|---:|---|
| 公开集主报（freeze_r9） | **0.035302** | 正式公开成绩 · v8 |
...
2026-08-02 协议合规复测：公开 NS64（1000/128）+ `fno_ns_public_demo.pt`；BIREN 单卡、64×64、batch=16、warmup=10、iters=50；chain 门禁 B=4 rel≈`9.98e-5` PASS。

| scope | grid_points/s | samples/s | ms/sample | ms/batch | peak MB |
|---|---:|---:|---:|---:|---:|
| pure forward | 1,605,792.317 | 392.039 | 2.550766 | 40.812 | 202.2 |
| with DataLoader | 1,442,203.341 | 352.100 | 2.840099 | 45.442 | 202.2 |

脚本：`fno_ns/benchmark_fno_batch16.py`（默认 public）；日志：`results/run_logs/fno_batch16_benchmark_public_ns64_2026-08-02.md`。历史 v2 旁注可用 `--legacy-v2`。grid point 按单通道 `H×W` 计算。
...
- `development_log.md`：≥28 段编号记录（精品抽查 24–28），覆盖 kernel、性能、模型/超参、数据、可视化和 BIREN 平台适配
