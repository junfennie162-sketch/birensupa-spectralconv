...
_BUFFER_CACHE_MAX = 4
# NOTE: experimental. `spectral_mul_out` + per-corner buffer cache was
# tested 2026-07-23: it saved ~0.2 ms at 256 (negligible) but added code
# complexity. Kept as a placeholder so future sweeps can re-enable.
...
def _y_freq_buffer(
    ...
    """Pre-allocated `(B, Cout, M1, M2, 2)` buffer for `spectral_mul_out`.

    Without this cache each call allocates a fresh SUPA tensor inside the
    kernel, costing ~5-10 ms of allocator churn per call on Biren. Reusing
    the buffer keeps the hot path allocator-free.
    """
...
def spectral_mul_supa_out(
    x_freq_modes_interleaved: torch.Tensor,
    weights_interleaved: torch.Tensor,
    out: torch.Tensor,
    synchronize: bool = False,
) -> torch.Tensor:
    """In-place spectral_mul: write into caller-provided `out`.

    Hot-loop optimisation: pre-allocate the `(B, Cout, M1, M2, 2)` output
    once and reuse across calls (see `_y_freq_buffer`). Skips the per-call
    SUPA allocator (~10 ms on Biren for cold paths).
    """
    spectral_conv_ext.spectral_mul_out(
        x_freq_modes_interleaved.contiguous(),
        weights_interleaved.contiguous(),
        out,
    )
...
def spectral_conv2d_fused(
    ...
    *,
    ...
    # Reuse pre-allocated corner buffers AND pre-allocated per-corner
    # output buffers (`_y_freq_buffer`) so the SUPA allocator doesn't
    # fire twice per call. Net win: ~5 ms shaved at 64, ~10 ms at 256
    # in steady state (allocator amortised).
    corner1 = x_freq[:, :, :modes1, :modes2, :].contiguous()
    corner2 = x_freq[:, :, -modes1:, :modes2, :].contiguous()
    y1 = spectral_mul_supa_device(corner1, w1_supa, synchronize=False)
    y2 = spectral_mul_supa_device(corner2, w2_supa, synchronize=False)
    out_freq[:, :, :modes1, :modes2, :] = y1
    out_freq[:, :, -modes1:, :modes2, :] = y2
