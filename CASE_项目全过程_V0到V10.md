...
| 项 | 现行结果（v10 · 2026-08-11 promote） |
|----|--------------------------------------|
| 赛事 | 书生国智科探挑战赛 · 壁仞飞翔杯 · 赛道五（模型与算子） |
| 队伍 | 翻斗花园 · 中北大学 |
| 必选 | 2D Spectral Convolution（SUPA + PyTorch Extension，方式二） |
| 选修 | FNO 预测二维不可压 Navier-Stokes **涡度**（公开 NS64） |
| Spectral 正确性 | worst rel ≈ **2.17×10⁻⁷**（阈值 1e-4）· 5/5 PASS |
| Spectral 性能 | idle **3.797 / 8.037 / 29.295 ms** @64/128/256（2026-08-14 复测） |
...
**一句话：**必选算子走 fused suFFT+SUPA 复数乘，正确性过关后把 formal ms 冻死；选修 FNO 在公开 NS64 上按「破 gate 才 promote」从 0.0418 抠到 0.035012，最后一跳是 Spectral-Refiner lite（只训 spectral + Sobolev H⁻¹ 损失）。
