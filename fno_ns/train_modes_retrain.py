#!/usr/bin/env python3
"""Official-split FNO train with selectable modes (default 12).

Uses the original global relative-L2 loss (same as train_official.py), which
historically reached ~0.005488 at modes=16. Saves to a modes-specific ckpt;
promote only if test L2 beats current demo on the same 1000/128 split.
"""
...
def main() -> int:
    ...
    out_ckpt = THIS_DIR / "checkpoints" / f"fno_ns_modes{args.modes}_best.pt"
    out_meta = THIS_DIR / "checkpoints" / f"fno_ns_modes{args.modes}_meta.json"
    ...
    # demo baseline on same split (modes=16 demo)
    demo_l2 = float("inf")
    if DEMO_CKPT.exists():
    ...
    if not out_ckpt.exists():
        torch.save({"model": best_state, "test_l2": best, "modes": args.modes}, out_ckpt)
    ...
    meta = {
        ...
        "note": "modes!=16 changes SpectralConv weight shapes; FNO inference must use same modes",
    }
    ...
    if improved and args.promote:
        ...
        # Promoting modes=12 into demo requires also updating default modes in loaders/tests.
        shutil.copy2(DEMO_CKPT, DEMO_CKPT.with_suffix(f".pt.pre_modes{args.modes}_backup"))
        shutil.copy2(out_ckpt, DEMO_CKPT)
        shutil.copy2(out_ckpt, BEST_CKPT)
        print({"promoted": True, "new_l2": best, "modes": args.modes}, flush=True)
    else:
