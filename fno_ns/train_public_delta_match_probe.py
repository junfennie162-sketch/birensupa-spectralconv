#!/usr/bin/env python3
"""Public NS64 · Δ-match probe (epochs≤4, freeze spectral).

Motivation (Error Autopsy D): step-1 error correlates strongly with temporal
increment magnitude q_t (ρ≈0.82). This probe adds an explicit frame-delta
relative loss, orthogonal to sample hard-reweight / PF / STLW / modes expand.

  loss = L2(pred, y) + λ_hf * HF(pred, y) + λ_δ * L2(pred-x_last, y-x_last)
...
def delta_match_loss(
    ...
    *,
    ...
    """y: [B, 1, H, W] or [B, T, H, W] — uses first future frame."""
    if y.dim() == 4 and y.shape[1] > 1:
        y1 = y[:, 0:1]
    else:
        y1 = y
    pred = predict(model, x, residual)
    loss = rel_l2_loss(pred, y1)
    if hf_weight > 0:
        loss = loss + hf_weight * highfreq_rel_loss(pred, y1)
    if lambda_delta > 0:
        x_last = x[:, -1:]
        d_pred = pred - x_last
        d_tgt = y1 - x_last
        loss = loss + lambda_delta * rel_l2_loss(d_pred, d_tgt)
    return loss
...
def main() -> None:
    ...
    ap.add_argument("--lambda-delta", type=float, default=0.5)
    ...
    ap.add_argument("--tag", type=str, default="delta_match_r1")
    args = ap.parse_args()
    ...
    best = baseline
    print(
        f"baseline={baseline:.8f} gate={gate:.8f} lambda_delta={args.lambda_delta} "
    ...
    lines = [
        f"# delta_match tag={args.tag} baseline={baseline:.8f} gate={gate:.8f} "
        f"lambda_delta={args.lambda_delta} freeze_spectral={freeze_spectral}"
    ]
    stale = 0
    t0 = time.time()
    epoch = -1
    ...
    summary = {
        ...
        "log": str(log_path),
        "promote": False,
        "note": "Autopsy q_t → Δ-match; no auto-promote; human confirm if beat_gate",
    }
