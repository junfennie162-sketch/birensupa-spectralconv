#!/usr/bin/env python3
"""Public NS64 · PF + Δ-match hybrid (epochs≤4, freeze spectral).

Combines two weak-positive probes that were previously orthogonal:
  - clean-anchor pushforward (pf_clean_r1 near-miss)
  - frame-delta match (delta_match_r1 tiny gain)

  loss = L2(pred1,y1) + λ_hf*HF(pred1,y1) + λ_δ*L2(Δpred1,Δy1)
       + λ_pf * (L2(pred2|sg(pred1),y2) + λ_hf*HF(...))
...
def pf_delta_hybrid_loss(
    ...
    *,
    ...
    if lambda_delta > 0:
        x_last = x[:, -1:]
        loss = loss + lambda_delta * rel_l2_loss(pred1 - x_last, y1 - x_last)
...
def main() -> None:
    ...
    ap.add_argument("--lambda-pf", type=float, default=0.75)
    ...
    ap.add_argument("--tag", type=str, default="pf_delta_r1")
    args = ap.parse_args()
    ...
    train_loader = DataLoader(
        RollAugDataset(SequenceVorticityDataset(train_data, 10, 2), augment=augment),
    ...
    print(
        ...
        f"lambda_pf={args.lambda_pf} lambda_delta={args.lambda_delta}",
        flush=True,
    )

    log_path = LOG_DIR / f"fno_public_{args.tag}_{time.strftime('%Y%m%d_%H%M%S')}.log"
    lines = [
        f"# pf_delta_hybrid tag={args.tag} baseline={baseline:.8f} gate={gate:.8f} "
    ...
    summary = {
        "task": "fno_public_pf_delta_hybrid",
        "tag": args.tag,
        ...
        "log": str(log_path),
        "promote": False,
        "note": "PF+Δ hybrid; no auto-promote; human confirm if beat_gate",
    }
