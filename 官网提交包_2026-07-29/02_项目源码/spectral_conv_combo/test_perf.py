...
def write_artifacts(perf_rows: list[dict]) -> None:
    ...
    summary["meta"]["notes"] = (
        "2026-07-25 protocol alignment complete. SpectralConv auto "
        f"{perf_rows[0]['forward_time_ms']:.3f}/"
        f"{perf_rows[1]['forward_time_ms']:.3f}/"
        f"{perf_rows[2]['forward_time_ms']:.3f} ms "
        f"(warmup={PERF_WARMUP}, iters={PERF_ITERS}). "
        "FNO checkpoint L2≈0.009516 on generated_ns_like_v2 (not public NS64)."
    )
