#!/usr/bin/env python3
"""P4: spectral_mul autograd vs pure torch einsum reference grads."""
...
def main() -> None:
    ...
    summary["spectral_conv"]["backward"] = {
        "status": "pass" if ok else "fail",
        "threshold": GRAD_REL_THRESHOLD,
        "worst_rel": worst,
        "cases": cases,
        "note": "SpectralMulFunction: SUPA forward + conj-transpose einsum backward",
    }
    summary.setdefault("optimization", {})
    summary["optimization"]["p4_backward"] = {
        "status": "done" if ok else "fail",
        "worst_grad_rel": worst,
    }
    SUMMARY_PATH.write_text(json.dumps(summary, indent=2) + "\n")

    log_path = RUN_LOG_DIR / f"spectral_backward_{day}.md"
    lines = [
        "# Spectral mul backward (P4)",
        "",
        f"- time_utc: {stamp}",
        f"- threshold: {GRAD_REL_THRESHOLD}",
        f"- worst_rel: {worst}",
        f"- ok: {ok}",
        "",
        "## Cases",
    ...
    for case in cases:
        lines.append(
            f"- {case['shape']}: fwd={case['fwd_rel']:.3e}, "
            f"gx={case['grad_x_rel']:.3e}, gw={case['grad_w_rel']:.3e}, ok={case['ok']}"
        )
    lines.append("")
    log_path.write_text("\n".join(lines))
