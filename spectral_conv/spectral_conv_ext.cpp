...
void spectral_mul_dual_scatter_out(
    ...
    // 1) Zero the (B, Cout, H, Wf, 2) buffer (the entire `out_freq`).
    //    This collapses the previous "_out_freq_buffer.zero_()" Python call
    //    (one full D2D-zero kernel) into the dual dispatch.
    out_freq.zero_();

    // 2) Pre-slice two strided (B, Cout, M1, M2, 2) views. `.contiguous()` is
    //    required because launch_spectral_mul assumes a packed buffer.
    auto out_top = out_freq.narrow(/*dim=*/2, /*start=*/0, /*length=*/M1)
                          .contiguous();
    auto out_bot = out_freq.narrow(/*dim=*/2, /*start=*/Wf - M1, /*length=*/M1)
                          .contiguous();
    ...
    check_status(launch_spectral_mul(static_cast<const float *>(x1.data_ptr()),
                                     static_cast<const float *>(w1.data_ptr()),
                                     static_cast<float *>(out_top.data_ptr()),
                                     static_cast<int>(B), static_cast<int>(Cin),
                                     static_cast<int>(Cout),
                                     static_cast<int>(M1), static_cast<int>(M2),
                                     nullptr),
                 "launch_spectral_mul(scatter/y_top)");
    check_status(launch_spectral_mul(static_cast<const float *>(x2.data_ptr()),
                                     static_cast<const float *>(w2.data_ptr()),
                                     static_cast<float *>(out_bot.data_ptr()),
                                     static_cast<int>(B), static_cast<int>(Cin),
                                     static_cast<int>(Cout2),
                                     static_cast<int>(M1), static_cast<int>(M2),
                                     nullptr),
                 "launch_spectral_mul(scatter/y_bot)");
}
