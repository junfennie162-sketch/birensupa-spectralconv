#!/usr/bin/env python3
"""Emit 16-row/256-thread fused inverse for 128."""
from __future__ import annotations

from pathlib import Path

ROOT = Path(__file__).resolve().parent
OUT = ROOT / "pruned_inv_fused128.su"
...
def transform_ifft_body(body: str) -> str:
    for group, off in enumerate(range(0, 128, 16)):
        marker = (
...
def main() -> None:
    ...
    irfft_src = (ROOT / "pruned_irfft_w128_vec4.su").read_text()
    ...
    kname = "__global__ void pruned_ifft_h_m16_fact16x8_h128_kernel"
...
extern "C" suError_t launch_pruned_irfft2_fused_mixed128(const float *in_freq, float *spatial,
                                                        ...
                                                        suStream_t stream) {{
    dim3 block(256);
    dim3 grid(static_cast<unsigned>(batch_size * channels * 8));
    pruned_irfft2_fused_mixed128_kernel<<<grid, block, 0, stream>>>(
